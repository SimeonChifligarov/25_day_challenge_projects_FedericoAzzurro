...
def extract_emails(
    ...
    if popular_domains_only:
        allowed: set[str] = {d.lower() for d in (popular_domains or POPULAR_EMAIL_DOMAINS)}
        emails = [email for email in emails if email.rsplit("@", 1)[1].lower() in allowed]
...
def main() -> None:
    popular_only: bool = _ask_yes_no("Keep only popular email domains? (y/n): ")
    list_emails("file.txt", popular_domains_only=popular_only)
